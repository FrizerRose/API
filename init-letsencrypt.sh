...
if ! [ -x "$(command -v docker-compose)" ]; then
  echo 'Error: docker-compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml  is not installed.' >&2
  exit 1
fi
...
echo "### Starting nginx ..."
docker-compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml  up --force-recreate -d nginx_container
echo
